...
# SOCKET=$TMP/startup.socket
# STATS=$TMP/stats.pipe
...
# if read -r -t 3 GAMESCOPE_X_DISPLAY GAMESCOPE_WL_DISPLAY <> "$SOCKET"; then
#   export DISPLAY="$GAMESCOPE_X_DISPLAY"
#   export GAMESCOPE_WAYLAND_DISPLAY="$GAMESCOPE_WL_DISPLAY"
# else
#   echo "gamescope failed" >> "${cfg.steamos.logsDir}/gamescope-stdout.log"
#   pkill "$GAMESCOPE_PID"
#   wait
#   exit 1
# fi
...
# RETURN=0
# wait -n $GAMESCOPE_PID $SLEEP_PID || RETURN=$?

# if [[ $RETURN = 127 ]]; then
#   echo >&2 "steamos: Interrupted while waiting on teardown, force-killing remaining tasks"
# fi

# for job in $(jobs -p); do
#   # Warn about unexpected things
#   if [[ $RETURN != 127 && $job = "$GAMESCOPE_PID" ]]; then
#     echo >&2 "steamos: gamescope timed out while exiting, killing"
#   elif [[ $RETURN != 127 && $job != "$SLEEP_PID" ]]; then
#     echo >&2 "steamos: unexpected background pid $job at teardown: "
...
#   steamos-runner = pkgs.writeShellApplication {
#     name = "steamos-runner";
#     runtimeInputs = with pkgs; [ gamescope mangohud ];
#     text = let
#       steam-runner = pkgs.writeScript "steam-runner" ''
#         mangoapp &
...
#       export SDL_VIDEO_MINIMIZE_ON_FOCUS_LOSS=0
#       export STEAM_USE_DYNAMIC_VRS=1
#       RADV_FORCE_VRS_CONFIG_FILE=$TMP/radv_vrs.XXXXXXXX
...
#       export LIBSEAT_BACKEND="logind"
#       export XKB_DEFAULT_LAYOUT="${config.console.keyMap}"
#       export MANGOHUD_CONFIGFILE="$TMP/mangohud.conf"
#       export STEAM_MULTIPLE_XWAYLANDS=1
#       export STEAM_USE_MANGOAPP=1
#       export STEAM_DISABLE_MANGOAPP_ATOM_WORKAROUND=1
#       export STEAM_MANGOAPP_PRESETS_SUPPORTED=1
...
#       echo "1x1" > "$RADV_FORCE_VRS_CONFIG_FILE"
#       echo "no_display" > "$MANGOHUD_CONFIGFILE"
#       gamescope -e --prefer-output "$OUT" -h "$IN_H" -H "$EX_H" -r "$RR" -F fsr -b -f --adaptive-sync --expose-wayland --xwayland-count 2 --steam -- ${steam-runner}
#     '';
#   };
#   steamos-pc = pkgs.writeShellScriptBin "steamos" ''
#     export OUT=DP-1
#     export IN_H=1080
#     export EX_H=1440
#     export RR=144
...
#     ${steamos-runner}/bin/steamos-runner
#   '';
#   consoleSession = (pkgs.writeTextDir "share/wayland-sessions/steamos.desktop" ''
#       [Desktop Entry]
#       Name=SteamOS
#       Comment=SteamOS Console Session
#       Exec=${steamos-tv}/bin/steamos-tv
...
# in {
#   options.nos.apps.games.steam = with types; {
#     enable = mkEnableOption "Enable steam.";
#     steamos = {
#       enable = mkEnableOption "Enable steamos with console session.";
#     };
...
#     hardware.xone.enable = true;
#     programs.gamemode.enable = true;
#     # programs.gamescope = mkMerge [
#     #   {
#     #     enable = true;
#     #   }
#     #   (mkIf cfg.steamos.enable {
#     #     capSysNice = true;
#     #   })
#     # ];
#     programs.steam = {
...
#         steamos-tv
#       ];
#     services.xserver.displayManager.sessionPackages = mkIf cfg.steamos.enable [ consoleSession ];
#   };
# }
