...
let
  ...
  cfg = config.nos.desktop.addons.picom;
in
{
  options.nos.desktop.addons.picom = {
    enable = mkEnableOption "Enable picom.";
  };

  config = mkIf cfg.enable {
    nos.home.extraOptions.services.picom = enabled // {
      backend = "glx";
      package = pkgs.nos.compfy;
      settings =
        let
          mkList = a: list: lists.concatMap (s: [ "${a} = '${s}'" ]) list;
        in
        {
          vsync = true;
          ...
          shadow-exclude = mkList "window_type" [
            "dock"
            "menu"
            "popup_menu"
            "utility"
